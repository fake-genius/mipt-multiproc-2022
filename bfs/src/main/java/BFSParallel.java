...
public class BFSParallel implements BFS {
    ...
    public BFSParallel(Graph graph) {
        this.graph = graph;
    ...
    public void bfs(int s) {
        ...
        while (frontier.size() != 0) {
            ...
            IntStream.range(0, frontier.size()).parallel().forEach(i -> {
                //System.out.println(Thread.currentThread().getName());
                        ArrayList<Integer> N = graph.getNeighbours(frontier3.get(i));
                        allNeighbours.set(i, N);
                        IntStream.range(0, degree(frontier3.get(i))).parallel().forEach(j -> {
            ...
            frontier = frontierPrime.parallelStream().filter(Objects::nonNull).collect(Collectors.toList());
        }
    }
